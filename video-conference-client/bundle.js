var LiveKitApp = (() => {
  ...
  var ConnectionError = class extends LivekitError {
    constructor(message, reason, status2, context) {
      super(1, message);
  ...
  var RTCEngine = class extends eventsExports.EventEmitter {
    ...
    constructor(options) {
      ...
      this.updateAndEmitDCBufferStatus = (kind) => {
        const status2 = this.isBufferStatusLow(kind);
        if (typeof status2 !== "undefined" && status2 !== this.dcBufferStatus.get(kind)) {
          this.dcBufferStatus.set(kind, status2);
          this.emit(EngineEvent.DCBufferStatusChanged, status2, kind);
        }
      };
  ...
  var RemoteParticipant = class _RemoteParticipant extends Participant {
    ...
    addTrackPublication(publication) {
      ...
      publication.on(TrackEvent.SubscriptionPermissionChanged, (status2) => {
        this.emit(ParticipantEvent.TrackSubscriptionPermissionChanged, publication, status2);
      });
      publication.on(TrackEvent.SubscriptionStatusChanged, (status2) => {
        this.emit(ParticipantEvent.TrackSubscriptionStatusChanged, publication, status2);
      });
  ...
  var Room = class _Room extends eventsExports.EventEmitter {
    ...
    maybeCreateEngine() {
      ...
      }).on(EngineEvent.DCBufferStatusChanged, (status2, kind) => {
        this.emit(RoomEvent.DCBufferStatusChanged, status2, kind);
      ...
      return __awaiter(this, arguments, void 0, function(kind, deviceId) {
        ...
        return (function* () {
          ...
          if (kind === "audioinput") {
            ...
            const isMuted2 = tracks.some((t) => {
              var _a2, _b2;
              return (_b2 = (_a2 = t.track) === null || _a2 === void 0 ? void 0 : _a2.isMuted) !== null && _b2 !== void 0 ? _b2 : false;
            });
            if (success && isMuted2) shouldTriggerImmediateDeviceChange = true;
          } else if (kind === "videoinput") {
    ...
    getOrCreateParticipant(identity, info) {
      ...
      }).on(ParticipantEvent.TrackSubscriptionStatusChanged, (pub, status2) => {
        this.emitWhenConnected(RoomEvent.TrackSubscriptionStatusChanged, pub, status2, participant);
      }).on(ParticipantEvent.TrackSubscriptionFailed, (trackSid, error) => {
        this.emit(RoomEvent.TrackSubscriptionFailed, trackSid, participant, error);
      }).on(ParticipantEvent.TrackSubscriptionPermissionChanged, (pub, status2) => {
        this.emitWhenConnected(RoomEvent.TrackSubscriptionPermissionChanged, pub, status2, participant);
